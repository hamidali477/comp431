...
def parseMailCommand(tokens):
    """Recursively parses mail-from command and returns error messages when errors
    in syntax are found
...
def parseRcptCommand(tokens):
    """Recursively parses rcpt-to command and returns error messages when errors
    in syntax are found
...
def parseDataCommand(command):
    """Parses data command and returns error messages when errors
...
def tokenizeSMTPCommand(command):
    """Tokenizes string of MAIL FROM or RCPT command that is input and returns as an array of
...
def createMessage(sender, recipients, message):
    """Creates file in ./forward directory and writes email message to it

    Parameters: sender (mailbox of sender), recipients (list of recipients), message from
        parseDataCommand() (list of each line of message)
    """
...
while True:
    ...
    if tokens[0] == "MAIL":
        ...
        if not hasMailFrom:
            ...
            recipient = parseRcptCommand(tokens)
            if recipient != 0:
